...
var (
	bootstrapServers, caLocation, topic string
	kp                                  *kafka.Producer
	kc                                  *kafka.Consumer
)
...
func init() {
	...
	producerConfig := &kafka.ConfigMap{"bootstrap.servers": bootstrapServers, "security.protocol": "SSL", "ssl.ca.location": caLocation}

	var err error
	kp, err = kafka.NewProducer(producerConfig)
	fmt.Println("Is it working here--->")
	if err != nil {
		log.Fatal("failed to create producer - ", err)
	}

	consumerConfig := &kafka.ConfigMap{"bootstrap.servers": bootstrapServers, "security.protocol": "SSL", "ssl.ca.location": caLocation, "group.id": "strimzi-tls-test-consumer-group"}
